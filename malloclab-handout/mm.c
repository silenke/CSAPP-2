...
/*
 * Segregated List，简单合并
 * 优化 realloc，考虑前面是空闲块的情况，另一种写法
 */
...
static void *realloc_coalesce(void *bp, size_t asize, int *nextfree);

static void printblock(void *bp);
static void checkheap(const char *line);
static void checkblock(void *bp);
static void printnode(const char *line);
...
/********************************/
    ...
    newbp = realloc_coalesce(bp, asize, &nextfree);
    if (nextfree) {
        realloc_place(newbp, asize);
        return newbp;
    }
    else if (newbp != bp) {
...
// 空闲块都在链表上
    ...
    if (!GET_ALLOC(HDRP(bp))) {
        int i = 0;
        void *findbp = SUCC_BLKP(get_root(get_root_index(GET_SIZE(HDRP(bp)))));
        for (; SUCC_BLKP(findbp) != 0; findbp = SUCC_BLKP(findbp)) {
            if (findbp == bp) {
                i = 1;
                break;
            }
...
static void printnode(const char *line)
{
    int i;
    void *bp;
    int size[] = {16,32,64,128,256,512,1024,2048,4096,0};
    for (i = 0; i < LISTNUM; i++) {
        bp = get_root(i);
        printf("List(%4d): root", size[i]);
        for (bp = SUCC_BLKP(bp); SUCC_BLKP(bp) != 0; bp = SUCC_BLKP(bp)) {
            printf(" -> [%u]", GET_SIZE(HDRP(bp)));
