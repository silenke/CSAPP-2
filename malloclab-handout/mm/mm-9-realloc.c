...
/********************************/
    ...
    nextbp = NEXT_BLKP(bp);
    newsize = oldsize + GET_SIZE(HDRP(nextbp));
    if (!GET_ALLOC(HDRP(nextbp)) && (asize <= newsize)) {
        removeb(nextbp);
    ...
    prevbp = PREV_BLKP(bp);
    newsize = oldsize + GET_SIZE(HDRP(prevbp));
    if (!GET_ALLOC(HDRP(prevbp)) && GET_ALLOC(HDRP(nextbp)) && (asize <= newsize)) {
        removeb(prevbp);
        PUT(HDRP(prevbp), PACK(newsize, 1));
        PUT(FTRP(prevbp), PACK(newsize, 1));
        memmove(prevbp, bp, oldsize);
        realloc_place(prevbp, asize);
        return prevbp;
    }

    newsize = oldsize + GET_SIZE(HDRP(prevbp)) + GET_SIZE(HDRP(nextbp));
    if (!GET_ALLOC(HDRP(prevbp)) && !GET_ALLOC(HDRP(nextbp)) && (asize <= newsize)) {
        removeb(prevbp);
        removeb(nextbp);
        PUT(HDRP(prevbp), PACK(newsize, 1));
