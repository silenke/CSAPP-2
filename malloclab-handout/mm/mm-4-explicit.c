...
/*
 * Explicit List，优化合并，FIFO，Address ordered
 */
...
// CHECKHEAP(__FUNCTION__);
    if ((root = mem_sbrk(8 * WSIZE)) == (void *)-1)
        return -1;

    PUT(root, 0);
    PUT(root + (1 * WSIZE), ADDR(root + DSIZE));
    PUT(root + (2 * WSIZE), ADDR(root));
    PUT(root + (3 * WSIZE), 0);
    PUT(root + (4 * WSIZE), 0);
    PUT(root + (5 * WSIZE), PACK(DSIZE, 1));
    PUT(root + (6 * WSIZE), PACK(DSIZE, 1));
    PUT(root + (7 * WSIZE), PACK(0, 1));
    heap_listp = root + (6 * WSIZE);
...
CHECKHEAP(__FUNCTION__);
    ...
    for (bp = SUCC_BLKP(root); SUCC_BLKP(bp) != 0; bp = SUCC_BLKP(bp)) {
...
#else
    for (; SUCC_BLKP(succbp) != 0; succbp = SUCC_BLKP(succbp)) {
        if (bp < succbp)
            break;
    }
    insertb2(bp, PRED_BLKP(succbp), succbp);
#endif
}
...
static void printblock(void *bp) 
{
    ...
    printf("%p: header: [%ld:%c] footer: [%ld:%c] %p %p\n", bp, 
           hsize, (halloc ? 'a' : 'f'), 
           fsize, (falloc ? 'a' : 'f'),
           predbp, succbp); 
    }
